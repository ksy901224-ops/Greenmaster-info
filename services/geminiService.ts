...
export const analyzeDocument = async (
  ...
  contentParts.push({
    text: `
      ...
      1. **Course Name Matching (매우 중요)**: 
         - 문서에서 추출한 골프장 이름이 위 "등록된 골프장 표준 명칭 목록"의 항목과 유사하다면(철자, 띄어쓰기, 접미사 차이 등), **반드시 목록에 있는 정확한 이름을 사용**하세요.
         - 예시: 문서에 "스카이뷰"라고 되어 있고 목록에 "스카이뷰 CC"가 있다면, "스카이뷰 CC"로 변환하여 출력하세요.
         - 예시: "레이크사이드 컨트리클럽" -> "레이크사이드 (Demo)" (목록에 있는 경우)
         - 대소문자, "GC", "CC", "Club" 등의 접미사 차이는 무시하고 매칭하세요.
