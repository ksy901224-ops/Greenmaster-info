...
const WriteLog: React.FC = () => {
  ...
  const startAiAnalysis = async () => {
    ...
    setIsAnalyzing(true);
    setAiResults(null);
    setAnalysisProgress('문서 텍스트 스캔 중...');
    try {
      ...
      setAnalysisProgress('전략 데이터 매핑 중...');
  ...
  const handleBulkPersonSubmit = async (e: React.FormEvent) => {
    ...
    try {
        ...
        alert(`${validEntries.length}명의 인물이 시스템에 등록되었습니다.`);
        navigate('/courses');
  ...
  const handleLogSubmit = (e: React.FormEvent) => {
    ...
    setTimeout(() => { setIsSubmitting(false); alert('저장되었습니다.'); navigate('/'); }, 500);
  };

  const getInputClass = () => `w-full rounded-xl border py-3 px-4 transition-all outline-none focus:ring-4 focus:ring-brand-500/5 focus:border-brand-500 bg-slate-50 border-slate-200 text-sm font-bold`;
  const getSelectClass = () => `w-full rounded-xl border py-3 px-4 transition-all outline-none focus:ring-4 focus:ring-brand-500/5 focus:border-brand-500 appearance-none bg-white border-slate-200 text-sm font-black`;
                ...
                <form onSubmit={handleLogSubmit} className="space-y-6">
                    ...
                    <div>
                        ...
                        <div className="flex gap-2">
                            ...
                            <button type="button" onClick={() => setIsCourseModalOpen(true)} className="p-3.5 bg-slate-900 text-white rounded-xl shadow-lg active:scale-95 transition-all"><PlusCircle size={20}/></button>
                        </div>
                    </div>
                    ...
                    <form onSubmit={handleBulkPersonSubmit} className="space-y-5">
                        <div className="space-y-4">
                            {personEntries.map((p, idx) => (
                                <div key={p.id} className="bg-slate-50/50 border border-slate-200 rounded-3xl p-6 relative group shadow-sm animate-in slide-in-from-top-2">
                                    <div className="flex justify-between items-center mb-5 border-b border-slate-100 pb-3">
                                        <span className="text-[10px] font-black bg-brand-100 text-brand-700 px-3 py-1 rounded-full uppercase tracking-widest">PERSON ENTRY #{idx + 1}</span>
                                    ...
                                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                                        <div><label className="block text-[9px] font-black text-slate-400 uppercase mb-2 ml-1">성명 *</label><input type="text" className={getInputClass()} value={p.name} onChange={e => updatePersonEntry(p.id, 'name', e.target.value)} required placeholder="이름 입력" /></div>
                                        <div><label className="block text-[9px] font-black text-slate-400 uppercase mb-2 ml-1">현재 직책</label><input type="text" className={getInputClass()} value={p.role} onChange={e => updatePersonEntry(p.id, 'role', e.target.value)} placeholder="예: 코스팀장" /></div>
                                        <div>
                                            <label className="block text-[9px] font-black text-slate-400 uppercase mb-2 ml-1 flex justify-between">소속 골프장 <button type="button" onClick={() => setIsCourseModalOpen(true)} className="text-brand-600 hover:underline">신규 등록</button></label>
                                            <div className="relative">
                                                <input list={`courses-p-${idx}`} className={getInputClass()} value={globalCourses.find(c => c.id === p.courseId)?.name || ''} onChange={e => { const f = globalCourses.find(c => c.name === e.target.value); updatePersonEntry(p.id, 'courseId', f ? f.id : ''); }} placeholder="골프장 검색..." />
                        ...
                        <div className="pt-8 flex flex-col items-center">
                            ...
                            <button type="submit" disabled={isSubmitting} className="w-full bg-slate-900 text-white py-5 rounded-2xl font-black text-lg shadow-xl active:scale-[0.98] transition-all flex justify-center items-center uppercase tracking-widest">
                                {isSubmitting ? <Loader2 className="animate-spin mr-2" /> : <CheckCircle className="mr-2" />} SAVE {personEntries.length} RECORDS
                            </button>
                        </div>
                    </form>
                </div>
                    ...
                    {!aiResults ? (
                        <div className="space-y-8">
                            <div className="text-center max-w-2xl mx-auto">
                                ...
                                <h2 className="text-3xl font-black text-slate-900 tracking-tight mb-4">AI 지능형 데이터 통합 분석</h2>
                                <p className="text-slate-500 leading-relaxed font-medium">보고서, 메일, 메모 캡처본을 업로드하세요. <br/>AI가 인적 관계와 업무 내용을 자동 추출하여 매핑합니다.</p>
                            </div>
                            {!isAnalyzing ? (
                                <div className="border-2 border-dashed border-slate-200 rounded-[3rem] p-20 text-center hover:border-brand-500 hover:bg-brand-50/10 cursor-pointer group transition-all duration-500" onClick={() => fileInputRef.current?.click()}>
                                    ...
                                    <p className="text-xs text-slate-400 mt-4 font-bold tracking-widest uppercase">Images, PDF, Text Logs</p>
                        ...
                        <div className="animate-in fade-in slide-in-from-bottom-6 duration-700">
                            <div className="flex justify-between items-end mb-10 border-b border-slate-100 pb-8">
                                <div><h2 className="text-3xl font-black text-slate-900 flex items-center tracking-tight"><ListChecks size={32} className="mr-3 text-brand-600"/> 데이터 검토 및 시스템 반영</h2><p className="text-slate-500 mt-2 font-medium">AI가 추출한 결과를 최종 확인하고 사내 지식 DB에 통합하세요.</p></div>
                            ...
                            <div className="space-y-12">
                                ...
                                {aiResults.extractedLogs.length > 0 && (
                                    <section>
                                        <h3 className="text-sm font-black text-slate-400 uppercase tracking-widest mb-6 flex items-center"><FileText size={16} className="mr-2"/> 업무 히스토리 추출 ({aiResults.extractedLogs.length})</h3>
                                        <div className="space-y-4">
                                ...
                                {aiResults.extractedPeople.length > 0 && (
                                    <section>
                                        <h3 className="text-sm font-black text-slate-400 uppercase tracking-widest mb-6 flex items-center"><HeartHandshake size={16} className="mr-2"/> 인적 자산 매핑 ({aiResults.extractedPeople.length})</h3>
                            ...
                            <div className="mt-16 pt-8 border-t border-slate-100 flex flex-col items-center">
                                <button onClick={commitAiResults} disabled={isSubmitting} className="w-full max-w-md bg-slate-900 text-white py-5 rounded-[2rem] font-black text-lg shadow-2xl hover:bg-slate-800 flex justify-center items-center active:scale-[0.98] transition-all">{isSubmitting ? <Loader2 size={24} className="animate-spin mr-3"/> : <Check size={24} className="mr-3 text-brand-400" />} 전체 데이터 시스템 반영</button>
                                <p className="text-xs text-slate-400 mt-4 font-bold tracking-tight">반영된 데이터는 즉시 전사 공유됩니다.</p>
                            </div>
                        </div>
                    )}
                </div>
        ...
        <div className="fixed inset-0 z-[200] flex items-center justify-center bg-slate-900/60 backdrop-blur-sm p-4 animate-in zoom-in-95 duration-200">
          <div className="bg-white rounded-[2.5rem] p-10 w-full max-w-lg shadow-2xl overflow-hidden flex flex-col">
             <div className="flex justify-between items-center mb-8 border-b border-slate-100 pb-6">
                 ...
                 <button onClick={() => setIsCourseModalOpen(false)} className="text-slate-400 hover:text-slate-900 transition-colors p-2 hover:bg-slate-100 rounded-full"><X size={28}/></button>
             </div>
             <div className="space-y-6 overflow-y-auto max-h-[60vh] custom-scrollbar px-1">
                 <div><label className="block text-[10px] font-black text-slate-400 uppercase tracking-widest mb-3 ml-1">공식 명칭 (Official Name)</label><input type="text" className={getInputClass()} value={newCourse.name} onChange={(e) => setNewCourse({...newCourse, name: e.target.value})} placeholder="예: 그린마스터 CC" /></div>
                 ...
                 <div><label className="block text-[10px] font-black text-slate-400 uppercase tracking-widest mb-3 ml-1">도로명 주소</label><input type="text" className={getInputClass()} value={newCourse.address} onChange={(e) => setNewCourse({...newCourse, address: e.target.value})} placeholder="정확한 주소를 입력하세요" /></div>
             </div>
